 ...
 *
 * For an excellent reference to programming SVGA cards see Finn Th\xF8gersen's
...
static void chk_ro()
{
  ...
  if (config.cpuemu<1) {
#endif
    volatile char *p = (char *) LOWMEM(0xa0077);
    my_fault = 1;
    *p = 0x99;
    if(my_fault) {
...
int vga_emu_protect_page(unsigned page, int prot)
{
  ...
  /* On non-x86, no sigsegv VGA write interception - never use PROT_NONE */
  if (prot != RW && prot != RO) prot = RW;
#endif
...
#if !defined(__x86_64__) && !defined(__i386__)
  /* On non-x86, no sigsegv VGA trap - always allow writes */
  if (config.cpuemu>1) prot = RW;
#endif
...
#ifdef X86_EMULATOR
#if defined(__x86_64__) || defined(__i386__)
  /* On x86, PROT_NONE lets the sigsegv handler intercept VGA writes */
  if (config.cpuemu>1)
      prot = VGA_EMU_NONE_PROT;
  else
#else
  /* On non-x86 (ARM64 etc), no sigsegv VGA trap - keep pages writable */
  if (config.cpuemu>1)
...
int vga_emu_init(int src_modes, ColorSpaceDesc *csd)
{
  ...
  if (!Video->update_screen) {
    ...
    if (Video->update_cursor) {
      ...
      memcpy((void *) LOWMEM(GFX_CHARS), vga_rom_08, 128 * 8);
      vbe_init(NULL);
   ...
   */
  {
    ...
    if((fd = open("/dev/mem", O_RDONLY)) >= 0) {
      if(
        ...
        read(fd, (void *) LOWMEM(GFX_CHARS), GFXCHAR_SIZE) != -1
      ) {
        ok = 1;
      }
      close(fd);
    ...
    if(!ok) {
      /* fallback font */
      for(i = 0; i < 1024; i++) *(unsigned char *)(LOWMEM(GFX_CHARS) + i) = font1[i];
    }
  }
#endif
