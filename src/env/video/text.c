...
/* Text screen base address.
 * The BIOS INT 10h code writes to lowmem (via screen_adr() in int10.c),
 * which uses LOWMEM(VGA_PHYS_TEXT_BASE) = lowmem_base + 0xB8000.
 * The VGA emulation allocates a separate vga.mem.base buffer, but in
 * text mode, the BIOS never writes there.  So for reading text content
 * we must use the lowmem mirror where the BIOS actually writes. */
extern char *lowmem_base;
static inline unsigned char *text_base(void) {
  return (unsigned char *)(lowmem_base + VGA_PHYS_TEXT_BASE +
                           vga.display_start);
}
...
static void restore_cell(unsigned cursor_location)
{
  ...
  sp = (Bit16u *)(text_base() + cursor_location);
...
{  
  ...
  if(check_cursor_location(vga.crtc.cursor_location - vga.display_start, &x, &y) &&
    ...
    Bit16u *cursor = (Bit16u *)(lowmem_base + VGA_PHYS_TEXT_BASE + vga.crtc.cursor_location);
...
{
  Bit16u *screen_adr = (Bit16u *)(text_base() +
				  y * vga.scan_len + x * 2);
...
*/
		bp = charbuff;
		start_off = (u_char *)sp - (u_char *)text_base();
		start_x = x;
#if CONFIG_SELECTION
