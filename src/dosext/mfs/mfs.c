...
#define VFAT_IOCTL_READDIR_BOTH  0
#define VFAT_IOCTL_READDIR_SHORT 0
static long vfat_ioctl = 0;
#define FAT_IOCTL_GET_ATTRIBUTES 0
#define FAT_IOCTL_SET_ATTRIBUTES 0
#define MSDOS_SUPER_MAGIC     0x4d44
#endif
...
static int cds_drive(cds_t cds)
{
  ...
  Debug0((dbg_fd, "cds_drive: cds=%p cds_base=%p offset=%td rec_size=%d drive=%d mod=%td\n",
    (void*)cds, (void*)cds_base, cds_offset, cds_record_size, drive, cds_offset % cds_record_size));
...
#else
  /* macOS doesn't mount real FAT filesystems for door game use */
  (void)name;
  return 0;
#endif
}
...
init_drive(int dd, char *path, int options)
{
  ...
  if (path == NULL) {
    Debug0((dbg_fd, "init_drive: NULL path for drive %d\n", dd));
    return (0);
  }
...
mfs_redirector(void)
{
  ...
  g_printf("MFS: mfs_redirector called, AX=%04x\n", LWORD(eax));
  ...
  switch (ret) {
  case FALSE:
    Debug0((dbg_fd, "dos_fs_redirect returned FALSE\n"));
    REG(eflags) |= CF;
    return 1;
  case TRUE:
    Debug0((dbg_fd, "dos_fs_redirect returned TRUE\n"));
    REG(eflags) &= ~CF;
    return 1;
  case UNCHANGED:
    Debug0((dbg_fd, "dos_fs_redirect returned UNCHANGED\n"));
    return 1;
  case REDIRECT:
    Debug0((dbg_fd, "dos_fs_redirect returned REDIRECT\n"));
    return 0;
  }
...
dos_fs_dev(state_t *state)
{
  ...
  g_printf("MFS: emufs operation: ebx=0x%08x\n", state->ebx);
  ...
  if (WORD(state->ebx) == 0x500) {
    ...
    g_printf("MFS: init 0x500: dos_ver=%d.%d lol=%p sda=%p\n",
             dos_major, dos_minor, (void*)lol, (void*)sda);
    if (running_DosC)
  ...
  if (WORD(state->ebx) == 0) {
    ...
    g_printf("MFS: redirect drive %d (first_free=%d)\n",
             drive_to_redirect, first_free_drive);
    ...
    *(ptr - 9) = 1;
	...
	cline[i++] = *(clineptr++);
      ...
      g_printf("MFS: redirect cline='%s'\n", cline);
      t = strtok(cline, " \n\r\t");
      if (t) {
	t = strtok(NULL, " \n\r\t");
      }
      g_printf("MFS: redirect t='%s' opt will follow\n", t ? t : "(null)");
	...
	opt = (p && (toupperDOS(p[0]) == 'R'));
      ...
      if (t == NULL) {
	/* emufs.sys Init sends a redirect with no path as
	 * a "server alive?" check.  Return success. */
	g_printf("MFS: no path in redirect, returning success (server alive)\n");
	SETWORD(&(state->eax), 1);
	return (REDIRECT);
      }
...
RedirectDisk(int dsk, char *resourceName, int ro_flag)
{
  ...
  *path = 0;
  ...
  if(dsk < 0 || dsk > lol_last_drive(lol)) {
    Debug0((dbg_fd, "RedirectDisk %c: FAILED - dsk=%d > lol_last_drive=%d\n", dsk + 'A', dsk, lol_last_drive(lol)));
    return 1;
  }
...
RedirectDevice(state_t * state)
{
  ...
  if (drive < 0 || drive > lol_last_drive(lol)) {
    Debug0((dbg_fd, "RedirectDevice: drive %c: FAILED\n", drive + 'A'));
...
{
  unsigned char *dev =
    (unsigned char *)LOWMEM(((devptr & 0xffff0000) >> 12) + (devptr & 0xffff));
...
dos_fs_redirect(state_t *state)
{
 ...
 dos_mode = 0;
  ...
  Debug0((dbg_fd, "dos_fs_redirect: FN=0x%02x\n", (int)LOW(state->eax)));
...
/*      Debug0((dbg_fd, "File data %02x %02x %02x\n",
  ...
  case CONTROL_REDIRECT:	/* 0x1e */
    ...
    {
      uintptr_t stack_addr = Addr_8086(state->ss, state->esp);
      u_short stack_word = *(u_short *)stack_addr;
      d_printf("MFS: CONTROL_REDIRECT: ss=%04x esp=%04x stack_addr=%p stack_word=%04x lowmem_base=%p\n",
        (unsigned)state->ss, (unsigned)(state->esp & 0xffff), (void*)stack_addr, stack_word, (void*)(uintptr_t)lowmem_base);
      /* also dump a few words from the stack */
      d_printf("MFS: stack dump: [SP]=%04x [SP+2]=%04x [SP+4]=%04x [SP+6]=%04x\n",
        *(u_short *)stack_addr,
        *(u_short *)(stack_addr + 2),
        *(u_short *)(stack_addr + 4),
        *(u_short *)(stack_addr + 6));
    }
    subfunc = LOW(*(u_short *) Addr(state, ss, esp));
    Debug0((dbg_fd, "CONTROL_REDIRECT: subfunc=%d\n", subfunc));
    switch (subfunc) {
    ...
    default:
      Debug0((dbg_fd, "CONTROL_REDIRECT: unknown subfunc %d\n", subfunc));
      SETWORD(&(state->eax), FUNC_NUM_IVALID);
      return (FALSE);
    }
    break;
