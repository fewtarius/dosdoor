...
unsigned short AllocateDescriptors(int number_of_descriptors)
{
  ...
  if (selector && in_dpmi && DPMI_CLIENT.LDT_ALIAS) {
...
static inline int check_verr(unsigned short selector)
{
  ...
  return ret;
#else
  /* Without real x86 segments, always return valid */
  return selector;
#endif
}
/*
...
#else
  {
    /* Software LAR: read access rights from descriptor table */
    int idx = selector >> 3;
    if (idx < MAX_SELECTORS && Segments[idx].used) {
      ret = Segments[idx].type;
    } else {
      ret = 0;
    }
  }
...
static int dpmi_debug_breakpoint(int op, struct sigcontext_struct *scp)
{
  /* Debug breakpoints not supported on non-Linux */
  (void)scp;
  return 0x21; /* unsupported */
}
#endif /* __linux__ */
...
int lookup_realmode_callback(Bit32u dos_addr, int *num)
{
  int i;
  Bit32u base;
  for (i = 0; i < in_dpmi; i++) {
    base = (Bit32u)(uintptr_t)SEG2LINEAR(DPMIclient[i].private_data_segment+RM_CB_Para_ADD);
    if ((dos_addr >= base) && (dos_addr < base + 0x10)) {
      *num = dos_addr - base;
      return i;
    }
  }
...
static void do_dpmi_int(struct sigcontext_struct *scp, int i)
{
  ...
  switch (i) {
    case 0x2f:
      switch (_LWORD(eax)) {
	case 0x1684:
	  D_printf("DPMI: Get VxD entry point, BX = 0x%04x (not supported)\n", _LWORD(ebx));
	  _es = _edi = 0;
	  return;
	case 0x1686:
...
/**/ e_printf("dpmi_fault: adjust prefixes to 66=%d,67=%d\n",
	...
	if (_eip==1+DPMI_SEL_OFF(DPMI_raw_mode_switch_pm)) {
	  ...
	  D_printf("DPMI: VxD call ignored, ax=%#x\n", _LWORD(eax));
...
{
  struct sigcontext_struct *scp;
  /* Compute DOS linear address for comparisons.
   * lina may be a host pointer (includes lowmem_base on macOS). */
  Bit32u dos_lina = (LWORD(cs) << 4) + LWORD(eip);
  if (!in_dpmi) {
  ...
  scp = &DPMI_CLIENT.stack_frame;
#ifdef TRACE_DPMI
  if ((debug_level('t')==0)||((int)dos_lina!=0xfc80a))
#endif
  D_printf("DPMI: realmode hlt: 0x%x\n", dos_lina);
  if (dos_lina == (DPMI_ADD + HLT_OFF(DPMI_return_from_dos))) {
 ...
 */
  ...
  } else if ((dos_lina>=(DPMI_ADD + HLT_OFF(DPMI_return_from_dosint))) &&
	     (dos_lina <(DPMI_ADD + HLT_OFF(DPMI_return_from_dosint)+256)) ) {
    int intr = dos_lina - (DPMI_ADD + HLT_OFF(DPMI_return_from_dosint));
    int update_mask = ~0;
  ...
  } else if (dos_lina == (DPMI_ADD + HLT_OFF(DPMI_return_from_realmode))) {
    struct RealModeCallStructure *rmreg = (struct RealModeCallStructure *)
      (GetSegmentBaseAddress(_es) +
      API_16_32(_edi));
  ...
  } else if (dos_lina == (DPMI_ADD + HLT_OFF(DPMI_return_from_dos_memory))) {
  ...
  } else if (dos_lina == (DPMI_ADD + HLT_OFF(DPMI_raw_mode_switch_rm))) {
  ...
  } else if ((dos_lina >= (DPMI_ADD + HLT_OFF(MSDOS_srm_start))) &&
	     (dos_lina < (DPMI_ADD + HLT_OFF(MSDOS_srm_end)))) {
    REG(eip) += 1;            /* skip halt to point to FAR RET */
  ...
  } else if ((dos_lina >= (DPMI_ADD + HLT_OFF(MSDOS_rpm_start))) &&
	     (dos_lina < (DPMI_ADD + HLT_OFF(MSDOS_rpm_end)))) {
    ...
    D_printf("DPMI: unhandled HLT: dos_addr=0x%x\n", dos_lina);
  }
}
