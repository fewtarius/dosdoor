...
int msdos_pre_extender(struct sigcontext_struct *scp, int intr)
{
	...
	case 0x17:		/* rename using FCB */
	    ...
	    REG(ds) = TRANS_BUFFER_SEG;
	    REG(edx) = 0;
	    MEMCPY_DOS2DOS(SEG_ADR_LINEAR(ds, dx),
	...
	case 0x29:		/* Parse a file name for FCB */
		...
		REG(ds) = seg;
		REG(esi) = 0;
		MEMCPY_DOS2DOS(SEG_ADR_LINEAR(ds, si),
			    (void *)GetSegmentBaseAddress(_ds) + D_16_32(_esi),
			    0x100);
		seg += 0x10;
		REG(es) = seg;
		REG(edi) = 0;
		MEMCPY_DOS2DOS(SEG_ADR_LINEAR(es, di),
	...
	case 0x53:		/* Generate Drive Parameter Table  */
		...
		REG(es) = seg;
		REG(ebp) = 0;
		MEMCPY_DOS2DOS(SEG_ADR_LINEAR(es, bp),
	...
	case 0x56:		/* rename file */
		...
		REG(ds) = seg;
		REG(edx) = 0;
		snprintf(SEG2UNIX(REG(ds)), MAX_DOS_PATH, "%s",
			     (char *)GetSegmentBaseAddress(_ds) + D_16_32(_edx));
		seg += 0x20;

		REG(es) = seg;
		REG(edi) = 0;
		snprintf(SEG2UNIX(REG(es)), MAX_DOS_PATH, "%s",
	...
	case 0x09:		/* Set Mouse Graphics Cursor */
	    ...
	    REG(es) = TRANS_BUFFER_SEG;
	    REG(edx) = 0;
	    MEMCPY_DOS2DOS(SEG_ADR_LINEAR(es, dx),
