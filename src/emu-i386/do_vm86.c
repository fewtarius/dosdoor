...
#define MAX_HALT_COUNT 3
  ...
  #define __SEG_ADR(type, seg, reg)  type((uintptr_t)(seg << 4) + LWORD(e##reg) + (uintptr_t)lowmem_base)
  done=0;
  is_rep=0;
  prefix66=prefix67=0;
  pref_seg=-1;
...
#endif
    set_debug_level('g', 1);
    fprintf(stderr, "FATAL: general protection fault opcode=%02x at %04x:%04x\n",
            *csp, _CS, org_eip);
    error("general protection at %p: %x\n", lina,*lina);
    show_regs(__FILE__, __LINE__);
    show_ints(0, 0x33);
    fatalerr = 4;
    fprintf(stderr, "FATAL: calling leavedos(4)\n");
...
{
	unsigned char * ssp = SEG2UNIX(LWORD(ss));
