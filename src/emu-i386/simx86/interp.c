 ...
 *
 * Simulates a period-correct CPU speed (4.77/10MHz) to prevent the
 * emulator from consuming 100% of a host core. Essential when dozens
 * of door game instances run concurrently on a BBS system.
 *
 * Every THROTTLE_BATCH instructions, we compare elapsed real time
 * against simulated time (based on target MHz). If we're ahead,
 * we nanosleep for the difference.
 *
 * At 10 MHz with ~4 cycles/instruction average:
 *   2.5M instructions/sec = 400ns per instruction
 *   100-instruction batch = 40us simulated
 *   Modern CPU executes batch in ~1us
 *   Sleep ~39us -> ~97.5% time sleeping
 */
#define THROTTLE_BATCH    100	/* instructions between throttle checks */
#define CYCLES_PER_INSN   4	/* average cycles per x86 instruction */

static void cpu_throttle(int insn_count, hitimer_t batch_start)
{
	hitimer_t now, elapsed_us, simulated_us;
	long long sleep_us;
	struct timespec ts;

	if (!config.cpu_mhz) return;

	now = GETTSC();
	elapsed_us = TSCtoUS(now - batch_start);

	/* simulated time for this batch at target MHz
	 * insn_count * cycles_per_insn / MHz = microseconds */
	simulated_us = (hitimer_t)insn_count * CYCLES_PER_INSN / config.cpu_mhz;

	sleep_us = (long long)simulated_us - (long long)elapsed_us;
	if (sleep_us > 0) {
		ts.tv_sec = 0;
		ts.tv_nsec = (sleep_us > 10000) ? 10000000 : (long)(sleep_us * 1000);
		nanosleep(&ts, NULL);
	}
}
			      ...
			      int btype)
{
	...
	case 0x31:
		...
		if (Fetch(P1)==JMPsid) {	/* eb xx */
		    ...
		    d_nt = (DOS_ADDR((long)P1) - LONG_CS) + dsp2;
...
unsigned char *Interp86(unsigned char *PC, int mod0)
{
	...
	hitimer_t throttle_start = GETTSC();
	...
	while (Running) {
		/* Check for pending signals/events (fullsim path).
		 * The JIT path has its own check inside the compiled
		 * code loop; fullsim needs this explicit check to
		 * exit the interpreter for signal processing. */
		if (CEmuStat & (CeS_SIGPEND|CeS_RPIC|CeS_STI)) {
			if (CEmuStat & CeS_SIGPEND) {
				CEmuStat = (CEmuStat & ~CeS_SIGPEND) | CeS_SIGACT;
				TheCPU.err = EXCP_SIGNAL;
				return PC;
			}
			if (CEmuStat & CeS_RPIC) {
				CEmuStat &= ~CeS_RPIC;
		...
		/* CPU speed throttle - check every THROTTLE_BATCH instructions */
		if (config.cpu_mhz && ++throttle_count >= THROTTLE_BATCH) {
			cpu_throttle(throttle_count, throttle_start);
			throttle_count = 0;
			throttle_start = GETTSC();
		}
		OVERR_DS = Ofs_XDS;
		OVERR_SS = Ofs_XSS;
		P0 = PC;	// P0 changes on instruction boundaries
		TheCPU.mode = mode = basemode;
		e_printf("DECODE: PC=%08lx opc=%02x\n", (long)PC, Fetch(PC));
		...
		if (!NewNode) {
			...
			while (temp && ((InterOps[Fetch(PC)]&1)==0) && (G=FindTree((long)PC)) != NULL) {
				...
				{ int m = mode | (G->flags<<16) | XECFND;
				  ...
				  P0 = PC = tmp; }
				if (TheCPU.err) return PC;
				/* throttle JIT blocks too */
				throttle_count += G->seqnum ? G->seqnum : 1;
				if (config.cpu_mhz && throttle_count >= THROTTLE_BATCH) {
					cpu_throttle(throttle_count, throttle_start);
					throttle_count = 0;
					throttle_start = GETTSC();
		...
		if ((PC==NULL)||(*((int *)PC)==0)) {
			/* PC points to zero-filled memory. In real mode this
			 * means executing ADD [BX+SI],AL forever. Detect
			 * runaway by counting consecutive zero fetches. */
			if (++zero_run == 1) {
				/* Log first occurrence with full state */
				error("CPU: executing zero-filled memory at %p (dos=0x%lx) CS=%04x IP=%04x\n",
					PC, DOS_ADDR((unsigned long)PC),
					TheCPU.cs, TheCPU.eip);
				e_printf("\n%s\nFetch %08x at %p mode %x\n",
					e_print_regs(),*((int *)PC),PC,mode);
			}
			if (zero_run > 64) {
				error("CPU runaway: 64+ zero-opcodes at dos 0x%lx, aborting\n",
					DOS_ADDR((unsigned long)PC));
...
/*ea*/	case JMPld: {
			...
			TheCPU.eip = jip;
			PC = (unsigned char *)(uintptr_t)HOST_ADDR(LONG_CS + jip);
...
/*c2*/	case RETisp: {
			...
			PC = (unsigned char *)(uintptr_t)HOST_ADDR(LONG_CS + TheCPU.eip); }
			break;
/*c3*/	case RET:
			CODE_FLUSH();
			POP(mode, &TheCPU.eip);
...
/*c8*/	case ENTER: {
			...
			sp = HOST_ADDR(LONG_SS + ((rESP - ds) & TheCPU.StackMask));
			bp = HOST_ADDR(LONG_SS + (rEBP & TheCPU.StackMask));
			PUSH(mode, &rEBP);
			frm = sp - HOST_ADDR(LONG_SS);
			if (level) {
				sp -= ds*level;
				while (--level) {
					bp -= ds;
...
/*ff*/	case GRP2wrm:
			...
			case Ofs_DX: {	/*2*/	 // CALL near indirect
					...
					TheCPU.eip = DOS_ADDR((long)PC) - LONG_CS;
					...
					PC = (unsigned char *)(uintptr_t)HOST_ADDR(LONG_CS + dp);
				}
				break;
...
/*df*/	case ESC7:  {
			...
			b &= 7;
			TheCPU.mode |= M_FPOP;
			e_printf("FPU_DECODE: PC=%08lx P0=%08lx opc=%02x exop=%02x b=%d\n",
				(long)PC, (long)P0, opc, exop, b);
			if (sim) {
...
/*0f*/	case TwoByteESC: {
			...
			if (opc2 == 0x02 || opc2 == 0x03)
			    dbug_printf("TwoByteESC: opc2=%02x REALMODE=%d cr[0]=%08x CS=%04x\n",
				    opc2, REALMODE(), TheCPU.cr[0], TheCPU.cs);
			switch (opc2) {
//
			...
			case 0x03: { /* LSL */ /* Load Segment Limit */
				...
				CODE_FLUSH();
				if (REALMODE()) {
				    dbug_printf("!!! LAR/LSL in REALMODE: cr[0]=%08x CS=%04x in_dpmi_emu=%d\n",
					    TheCPU.cr[0], TheCPU.cs, in_dpmi_emu);
				    fflush(dbg_fd);
				    goto illegal_op;
				}
				PC += ModRMSim(PC+1, mode) + 1;
...
not_implemented:
	dbug_printf("!!! Unimplemented %02x %02x %02x at %p PC-offset=%ld\n",opc,PC[1],PC[2],PC,
		    (long)(PC - (unsigned char *)(uintptr_t)HOST_ADDR(LONG_CS)));
	fflush(dbg_fd);
...
not_permitted:
	if (debug_level('e')>1) {
		e_printf("not_permitted %02x at IP=%04lx PC bytes: %02x %02x %02x %02x %02x\n",
			    opc,
			    (unsigned long)(PC - (unsigned char *)(uintptr_t)HOST_ADDR(LONG_CS)),
...
illegal_op:
	{
		unsigned long real_ip = (unsigned long)(PC - (unsigned char *)(uintptr_t)HOST_ADDR(LONG_CS));
		dbug_printf("!!! Illegal op %02x %02x %02x at CS:IP=%04x:%04lx (linear %08lx, host PC=%p)\n",
			opc,PC[1],PC[2],TheCPU.cs,real_ip,(unsigned long)(LONG_CS+real_ip),(void*)PC);
	}
	fflush(dbg_fd);
