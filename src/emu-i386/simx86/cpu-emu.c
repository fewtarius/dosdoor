...
static void Cpu2Reg (void)
{
  ...
  vm86s.regs.eip = DOS_ADDR(return_addr) - LONG_CS;
  /*
...
static inline int e_revectored(int nr, struct revectored_struct * bitmap)
{
	...
	return nr;
#else
	return (((unsigned char *)bitmap)[nr / 8] & (1 << (nr % 8))) ? -1 : 0;
#endif
}
...
static int handle_vm86_trap(int *error_code, int trapno)
{
	...
	e_do_int(trapno, SEG2UNIX(_SS), _SP);
	return -1;
}
...
static int handle_vm86_fault(int *error_code)
{
	...
	if (op==0xcd) {
		...
		_IP += 2;
		if (debug_level('e')>1)
		    e_printf("handle_vm86_fault: INT %02x at CS:IP=%04x:%04x\n",
			    intno, _CS, _IP-2);
		...
		e_printf("EMU86: unhandled opcode %#x at CS:IP=%04x:%04x (linear %#lx)\n",
			op, _CS, _IP, (unsigned long)(csp + _IP));
		return VM86_UNKNOWN;
	}
}
  ...
  do {
    ...
    do {
      ...
      in_vm86_emu = 1;
      {
        ...
        interp_count++;
        e_printf("INTERP[%d]: enter=%08lx\n",interp_count, trans_addr);
        return_addr = (long)Interp86((char *)HOST_ADDR(trans_addr), mode);
        e_printf("INTERP[%d]: exit=%08lx err=%d\n",interp_count, return_addr,TheCPU.err-1);
      }
      xval = TheCPU.err;
      in_vm86_emu = 0;
      ...
      trans_addr = DOS_ADDR(return_addr);
    }
    while (xval==0);
	...
	switch (xval) {
		...
		if ((REG(cs)&0xf000)==config.vbios_seg) {
	      ...
	      break;
	    case EXCP06_ILLOP: {
		/* Invalid opcode - deliver as INT 6 to DOS */
		error("EMU86: invalid opcode at CS:IP=%04x:%04x\n",
		      LWORD(cs), LWORD(eip));
		in_vm86=0;
		leavedos(1);
	      }
...
int e_dpmi(struct sigcontext_struct *scp)
{
  ...
  TheCPU.cr[0] |= 1;
  dbug_printf("DPM86: entering DPMI, cr[0]=%08x\n", TheCPU.cr[0]);
