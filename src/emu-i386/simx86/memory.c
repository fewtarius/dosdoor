...
/* Track pages that get frequent data write faults. Pages exceeding
 * the threshold are excluded from mprotect to avoid the infinite
 * protect-fault-unprotect-recompile-protect cycle that occurs when
 * code and frequently-written data share a page. */
#define HOT_PAGE_THRESHOLD 16
#define HOT_PAGE_SLOTS 256
static unsigned short hot_page_count[HOT_PAGE_SLOTS];

void e_hot_page_fault(unsigned long cr2)
{
	unsigned int page = (unsigned int)(cr2 >> 12);
	if (page < HOT_PAGE_SLOTS) {
		if (hot_page_count[page] < 0xffff)
			hot_page_count[page]++;
	}
}

static int e_is_hot_page(caddr_t addr)
{
	unsigned int page = (unsigned int)((unsigned long)addr >> 12);
	if (page < HOT_PAGE_SLOTS && hot_page_count[page] >= HOT_PAGE_THRESHOLD)
		return 1;
	return 0;
}

int e_is_hot_page_cr2(unsigned long cr2)
{
	unsigned int page = (unsigned int)(cr2 >> 12);
	if (page < HOT_PAGE_SLOTS && hot_page_count[page] >= HOT_PAGE_THRESHOLD)
		return 1;
...
int e_mprotect(caddr_t addr, size_t len)
{
	...
	if (lowmem_base && abeg >= lowmem_base + 0xb0000 && abeg < lowmem_base + 0xc0000) {
	    fprintf(stderr, "e_mprotect: VIDEO area addr=%p (offs=0x%lx)\n",
	            abeg, (unsigned long)(abeg - lowmem_base));
	}
	/* Skip actual mprotect for pages that receive frequent data
	 * write faults. The compiled code from these pages will still
	 * work, but we accept the risk that self-modifying code on
	 * hot pages won't be detected until the next tree cleanup. */
	if (e_is_hot_page(abeg)) {
	    if (debug_level('e')>1)
		e_printf("MPMAP: skip hot page %p\n", abeg);
	    return AddMpMap(abeg, aend, 1);
	}
...
int e_munprotect(caddr_t addr, size_t len)
{
	...
	/* Hot pages: the real page permissions are already RWX since
	 * e_mprotect skips the actual mprotect call. Just update the
	 * map tracking. For the SIGSEGV handler case (page was actually
	 * protected before becoming hot), we still do the real syscall
	 * since the page needs to be made writable. We detect this by
	 * checking if we're in a signal handler (fault_cnt > 0). */
	if (e_is_hot_page(abeg) && !fault_cnt) {
	    return AddMpMap(abeg, aend, 0);
	}
