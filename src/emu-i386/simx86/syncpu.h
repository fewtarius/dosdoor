...
/* read segment base (BoundL) and convert to host address.
 * On Linux, lowmem_base alias at address 0 means DOS addr == host addr.
 * On macOS, no alias exists so we must add lowmem_base offset. */
#ifdef __linux__
#define CPUSEGBASE(o)	((unsigned long)CPULONG(o))
#else
#define CPUSEGBASE(o)	((unsigned long)CPULONG(o) + (unsigned long)lowmem_base)
#endif
...
/* HOST_ADDR: convert a DOS linear address to a host pointer.
 * DOS_ADDR: convert a host pointer back to a DOS linear address.
 * On Linux, alias-at-0 means these are identity transforms.
 * On macOS, must add/subtract lowmem_base. */
#ifdef __linux__
#define HOST_ADDR(a)	((unsigned long)(a))
#define DOS_ADDR(p)	((unsigned long)(p))
#else
#define HOST_ADDR(a)	((unsigned long)(a) + (unsigned long)lowmem_base)
#define DOS_ADDR(p)	((unsigned long)(p) - (unsigned long)lowmem_base)
#endif
