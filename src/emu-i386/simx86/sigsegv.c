...
{
  /* Always log pagefaults with emulated CPU state */
  if (_trapno == 0x0e) {
    dbug_printf("CPUEMU PAGEFAULT: cr2=%08lx err=%08lx rip=%08lx\n", _cr2, _err, _rip);
    dbug_printf("  LONG_CS=%08x LONG_DS=%08x LONG_SS=%08x LONG_ES=%08x\n",
      TheCPU.cs_cache.BoundL, TheCPU.ds_cache.BoundL,
      TheCPU.ss_cache.BoundL, TheCPU.es_cache.BoundL);
    dbug_printf("  eax=%08x ebx=%08x ecx=%08x edx=%08x\n",
      TheCPU.eax, TheCPU.ebx, TheCPU.ecx, TheCPU.edx);
    dbug_printf("  esp=%08x ebp=%08x esi=%08x edi=%08x eip=%08x\n",
      TheCPU.esp, TheCPU.ebp, TheCPU.esi, TheCPU.edi, TheCPU.eip);
    dbug_printf("  AR1=%08lx AR2=%08lx mode=%04x\n",
      (unsigned long)AR1.d, (unsigned long)AR2.d, TheCPU.mode);
  }
  ...
  if (_trapno==0x0e) {
	...
	if ((_err&0x0f)==0x07) {
		...
		/* Track data write faults per page so we can stop
		 * protecting pages with heavy data write traffic */
		e_hot_page_fault(_cr2);
