...
static void Gen_sim(int op, int mode, ...)
{
	...
	case S_DI_IMM: {
		int v = va_arg(ap,int);
		if (vga_write_access(DOS_ADDR(AR1.d))) {
			GTRACE0("S_DI_IMM_VGA");
			if (!vga_access(DOS_ADDR(AR1.d))) break;
			e_VgaWrite(DOS_ADDR(AR1.d), v, mode); break;
		}
		if (mode&MBYTE) {
	...
	case L_MOVZS: {
		...
		if (vga_read_access(DOS_ADDR(AR1.d))) {
		    if (vga_access(DOS_ADDR(AR1.d)))
			DR1.d = e_VgaRead(DOS_ADDR(AR1.d), mode);
		    else
			DR1.d = 0xffffffff;
		    if (rcod) {
			if (mode & MBYTX)
	...
	case L_VGAWRITE:
		if (vga_write_access(DOS_ADDR(AR1.d))) {
			GTRACE0("L_VGAWRITE");
			if (!vga_access(DOS_ADDR(AR1.d))) break;
			e_VgaWrite(DOS_ADDR(AR1.d), DR1.d, mode); break;
		}
		GTRACE0("S_DI");
		if (mode&MBYTE) {
		    *AR1.pu = DR1.b.bl;
...
/* POP derived (sub-)sequences: */
	...
	case O_MOVS_SetA:
		...
		if (mode&ADDR16) {
		    if (mode&MOVSSRC) {
	    		AR2.d = CPUSEGBASE(OVERR_DS);
			DR2.d = CPUWORD(Ofs_SI); /* for overflow calc */
			AR2.d += DR2.d;
		    }
		    if (mode&MOVSDST) {
	    		AR1.d = CPUSEGBASE(Ofs_XES);
		    ...
		    if (mode&MOVSSRC) {
	    		AR2.d = CPUSEGBASE(OVERR_DS) + CPULONG(Ofs_ESI);
		    }
		    if (mode&MOVSDST) {
	    		AR1.d = CPUSEGBASE(Ofs_XES) + CPULONG(Ofs_EDI);
		    }
	...
	case O_MOVS_MovD: {
		...
		if(i == 0)
		    break;
		v = vga_read_access(DOS_ADDR(AR2.d)) | (vga_write_access(DOS_ADDR(AR1.d)) << 1);
		if (v) {
		    int op;
	...
	case O_MOVS_LodD: {
		...
		if (vga_read_access(DOS_ADDR(AR2.d))) {
		    while (i--) {
			if (vga_access(DOS_ADDR(AR2.d)))
			    DR1.d = e_VgaRead(DOS_ADDR(AR2.d), mode);
			else
			    DR1.d = 0xffffffff;
			AR2.pu += df;
			if (!(mode&MBYTE)) {
	...
	case O_MOVS_StoD: {
		...
		if (vga_write_access(DOS_ADDR(AR1.d))) {
		    while (i--) {
		        if (vga_access(DOS_ADDR(AR1.d)))
			    e_VgaWrite(DOS_ADDR(AR1.d), DR1.d, mode);
			AR1.pu += df;
			if (!(mode&MBYTE)) {
			    AR1.pu += df;
...
static unsigned char *CloseAndExec_sim(unsigned char *PC, TNode *G, int mode, int ln)
{
	...
	if (signal_pending || TheCPU.sigalrm_pending) {
