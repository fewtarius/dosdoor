...
unsigned short *screen_adr(int page)
{
  ...
  unsigned short *base = IS_SCREENMODE_MDA ?
    (unsigned short *)LOWMEM(MDA_PHYS_TEXT_BASE) :
    (unsigned short *)LOWMEM(VGA_PHYS_TEXT_BASE);
...
int int10(void) /* with dualmon */
{
  ...
  switch(HI(ax)) {
	...
	if (LO(cx) & 1) {
	  ...
	  MEMCPY_2DOS((void *)SEGOFF2LINEAR(_ES, base), buf, sizeof(buf));
	  base += sizeof(buf);
	}
	if (LO(cx) & 2) {
	  MEMCPY_DOS2DOS((void *)SEGOFF2LINEAR(_ES, base), (char *)0x449, 96);
	  base += 96;
	}
	if (LO(cx) & 4) {
	...
	if (LO(cx) & 1) {
	  unsigned char buf[0x46];
	  unsigned crtc, ind;
	  /* Note: this MEMCPY_2UNIX needs a DOS linear address, not host pointer */
	  MEMCPY_2UNIX(buf, (void *)SEGOFF2LINEAR(_ES, base), sizeof(buf));
	  base += sizeof(buf);
	...
	if (LO(cx) & 2) {
	  MEMCPY_DOS2DOS((char *)0x449, (void *)SEGOFF2LINEAR(_ES, _BX), 96);
	  base += 96;
	}
	if (LO(cx) & 4) {
	  unsigned char buf[0x304];
