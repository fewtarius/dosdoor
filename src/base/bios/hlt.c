...
void hlt_handle(void)
{
   ...
   * On Linux (alias-at-0), lina IS the DOS linear address.
   * On macOS, lina includes lowmem_base and can't be compared
   * against DOS address constants like BIOS_HLT_BLK, DPMI_ADD, etc. */
  Bit32u  dos_addr = (LWORD(cs) << 4) + LWORD(eip);
  Bit32u  offs = dos_addr;
  int rmcb_client, rmcb_num;

#if defined(X86_EMULATOR) && defined(SKIP_EMU_VBIOS)
  if ((config.cpuemu>1) && (dos_addr == CPUEMUI10_ADD)) {
...
#endif
    {
      int hid = hlt_handler_id[offs];
      unsigned int arg = offs - hlt_handler[hid].start_addr;
      hlt_handler[hid].func(arg);
    }
  }
  else if (dos_addr == CBACK_ADD) {
    /* we are back from a callback routine */
    callback_return();
  }
  else if (dos_addr == Mouse_HLT_ADD) {
    int33_post();
  }
  else if (dos_addr == (DPMI_ADD + HLT_OFF(DPMI_dpmi_init))) {
    /* The hlt instruction is 6 bytes in from DPMI_ADD */
    _IP += 1;	/* skip halt to point to FAR RET */
    CARRY;
    dpmi_init();
  }
  else if ((dos_addr >= DPMI_ADD) &&
	   (dos_addr < (DPMI_ADD + (Bit32u)(uintptr_t)DPMI_dummy_end-
  ...
  else if ((rmcb_client = lookup_realmode_callback(dos_addr, &rmcb_num)) != -1) {
...
#endif
    /* Unregistered HLT - this is a CPU idle instruction.
     * In real hardware, HLT stops the CPU until the next interrupt.
     * We yield CPU time to avoid busy-spinning. */
    {
      struct timespec ts = {0, 10000000}; /* 10ms - roughly half a timer tick */
      nanosleep(&ts, NULL);
    }
    _IP += 1;
  }
