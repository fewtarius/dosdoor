...
static void process_master_boot_record(void)
{
   ...
   */
   ...
   /* int13 writes boot sector through lowmem_base, bypassing the mprotect
    * on the alias-at-0 mapping. Invalidate any stale JIT nodes at 0x7c00. */
   e_invalidate((char *)(uintptr_t)0x7c00, 0x200);
   ...
   dbug_printf("BOOT: MBR processed, jumping to 0000:7c00, bootrec magic=%04x, first bytes=%02x %02x %02x %02x %02x\n",
     bootrec->bootmagic, ((unsigned char*)bootrec)[0], ((unsigned char*)bootrec)[1],
     ((unsigned char*)bootrec)[2], ((unsigned char*)bootrec)[3], ((unsigned char*)bootrec)[4]);
}
...
int dos_helper(void)
{
  g_printf("dos_helper: AL=0x%02x AH=0x%02x\n", LO(ax), HI(ax));
  switch (LO(ax)) {
...
*/
  ...
  case 0x83:
    ...
    break;			/* no event wait */
  case 0x84:
    /* joystick bios read - removed, return no joystick */
    CARRY;
    break;
  case 0x85:
...
---------------------------------------------
	...
	if (REG(ebx) < system_memory_map_size) {
	  ...
	  MEMCPY_2DOS((void *)SEGOFF2LINEAR(_ES, _DI), (char *)system_memory_map + REG(ebx),
		      REG(ecx));
	  REG(ebx) += REG(ecx);
	} else
	  REG(ecx) = 0;
...
void redirect_devices(void)
{
  ...
  for (i = 0; i < MAX_HDISKS; i++) {
    if(hdisktab[i].type == DIR_TYPE && hdisktab[i].fatfs) {
      ...
      s[255] = 0;
      ds_printf("redirect_devices: drive %c: => %s\n", i + 'C', s);
      j = RedirectDisk(i + 2, s, hdisktab[i].rdonly);
      ds_printf("redirect_devices: drive %c: result=%d\n", i + 'C', j);
...
static int redir_it(void)
{
  ...
  switch(redir_state) {
    case 1:
      if(HI(ax) == 0x3d) {
        ds_printf("redir_it: state 1, running_DosC=%d\n", running_DosC);
        /*
