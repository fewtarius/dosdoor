...
int int13(void)
{
  ...
  case 2:			/* read */
    ...
    LWORD(eax) = res >> 9;
    REG(eflags) &= ~CF;
    /* disk read writes through lowmem_base, invalidate JIT cache
     * using DOS linear address (for alias-at-0 JIT node keys) */
    e_invalidate((char *)(uintptr_t)SEG_ADR_LINEAR(es, bx), res);
  ...
  case 0x42: {                  /* IBM/MS Extensions, read */
    ...
    buffer = (char *)LOWMEM(((unsigned long)diskaddr->buf_seg << 4)+diskaddr->buf_ofs);
