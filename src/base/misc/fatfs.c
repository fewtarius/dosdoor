...
void fatfs_done(struct disk *dp)
{
  ...
  /* Just null out the pointer - don't free individual allocations.
   * During abnormal shutdown (e.g. CPU emulation error), the heap may
   * be corrupted if the emulator wrote out of bounds. */
  dp->fatfs = NULL;
}
...
int fatfs_read(fatfs_t *f, unsigned char *buf, unsigned pos, int len)
{
  ...
  while(l) {
    ...
    e_invalidate((char *)(uintptr_t)buf, 0x200);
    buf += 0x200; pos++; l--;
  }
...
void fdkernel_boot_mimic(void)
{
  int f, size;
  char *bootfile;
  void *loadaddress = (void *)LOWMEM(0x600);
  fatfs_t *fs = get_fat_fs_by_drive(HI(ax));

  dbug_printf("BOOT: fdkernel_boot_mimic called, drive=%02x\n", HI(ax));
  ...
  lseek(f, 0, SEEK_SET);
  {
    int ret = dos_read(f, (char*)0x600, size);
    unsigned char *p = (unsigned char *)loadaddress;
    dbug_printf("BOOT: dos_read returned %d for size %d\n", ret, size);
    dbug_printf("BOOT: loadaddress=%p, first bytes=%02x %02x %02x %02x %02x %02x\n",
                loadaddress, p[0], p[1], p[2], p[3], p[4], p[5]);
    fflush(dbg_fd);
  }
  close(f);
  LWORD(cs) = LWORD(ds) = LWORD(es) = 0x60;  /* segment for DOS address 0x600 */
  LWORD(eip) = 0;
...
"Press any key to return to Linux...\r\n";
  ...
  b[0x01] = 0x3e;
  b[0x02] = 0x90;
  memcpy(b + 0x40, boot_prog, BOOT_PROG_SIZE);
  strcpy(b + 0x40 + BOOT_PROG_SIZE, msg);
  b[0x1fe] = 0x55;
  b[0x1ff] = 0xaa;

  t_o = (0x40 + BOOT_PROG_SIZE + strlen(msg) + 1 + 3) & ~3;
  d0 = b + t_o;
  d1 = d0 + 0x14;
  ...
  t_o = 0x7c00 + 0x40 + BOOT_PROG_SIZE - 5;
  ...
  switch(f->sys_type) {
    ...
    default:			/* no system */
      strcpy(b + 0x40 + BOOT_PROG_SIZE, msg1);
      t_o = (0x40 + BOOT_PROG_SIZE + strlen(msg1) + 1 + 3) & ~3;
      d0 = b + t_o;
