 ...
 * usage() and the manpage in sync!
 */
static char *get_option_impl(char *key, int with_arg, int *argc, char **argv)
{
    char *p;
    char *basename;
...
#define get_option(key, with_arg) get_option_impl(key, with_arg, argc, argv)
...
#ifdef __APPLE__
    /* macOS: no /proc/cpuinfo - hardcode reasonable defaults */
    config.realcpu = CPU_586;
    config.mathco = 1;
    config.smp = 1;
    {
        /* On ARM64/Apple Silicon, GETTSC() reads cntvct_el0 which runs at
         * hw.tbfrequency (typically 24 MHz), NOT the CPU clock speed.
         * All timing (sigEMUdelta, cpu_spd, etc.) must match GETTSC rate. */
        long long freq = 0;
        size_t sz = sizeof(freq);
#if defined(__aarch64__)
        /* Use the actual timer frequency that GETTSC reads */
        long long tbfreq = 0;
        size_t tbsz = sizeof(tbfreq);
        if (sysctlbyname("hw.tbfrequency", &tbfreq, &tbsz, NULL, 0) == 0 && tbfreq > 0) {
            freq = tbfreq;
        } else {
            freq = 24000000LL; /* Apple Silicon default */
        }
#else
        if (sysctlbyname("hw.cpufrequency", &freq, &sz, NULL, 0) == 0 && freq > 0) {
            /* x86 Mac: TSC runs at CPU frequency */
        } else {
            freq = 2000000000LL;
        }
#endif
        config.cpu_spd = (LLF_US * 1000000) / freq;
        config.cpu_tick_spd = (LLF_TICKS * 1000000) / freq;
        config.CPUSpeedInMhz = (int)(freq / 1000000);
        if (config.CPUSpeedInMhz < 1) config.CPUSpeedInMhz = 1;
        warn("macOS: CPU speed is %d MHz (timer freq %lld Hz)\n",
             config.CPUSpeedInMhz, freq);
    }
    return;
#endif /* __APPLE__ */
