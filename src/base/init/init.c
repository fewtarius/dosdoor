...
static inline void dbug_dumpivec(void)
{
  ...
  for (i = 0; i < 256; i++) {
    ...
    dbug_printf("%02x %08x", i, ((unsigned int *) LOWMEM(0))[i << 1]);
    for (j = 0; j < 8; j++)
      dbug_printf(" %02x", ((unsigned char *) LOWMEM(BIOSSEG * 16 + 16 * i))[j]);
    dbug_printf("\n");
  }
}
#endif
...
void map_video_bios(void)
{
  ...
  if (config.mapped_bios) {
    ...
    load_file("/dev/mem", GFX_CHARS, (char *) LOWMEM(GFX_CHARS), GFXCHAR_SIZE);

    memcheck_addtype('V', "Video BIOS");
    memcheck_reserve('V', (size_t)(config.vbios_seg << 4), VBIOS_SIZE);
...
static inline void map_custom_bios(void)
{
  ...
  n = BIOS_SYM(bios_f000_endpart1) - BIOS_SYM(bios_f000);
  ptr = (u_char *) LOWMEM(BIOSSEG << 4);
  g_printf("BIOS: copying %lu bytes to %p (lowmem_base=%p, offset=0x%x)\n",
           n, ptr, lowmem_base, BIOSSEG << 4);
  memcpy(ptr, bios_f000, n);

  n = BIOS_SYM(bios_f000_end) - BIOS_SYM(bios_f000_part2);
  ptr = (u_char *) LOWMEM((BIOSSEG << 4) + (BIOS_SYM(bios_f000_part2) - BIOS_SYM(bios_f000)));
  g_printf("BIOS: copying part2 %lu bytes to %p\n", n, ptr);
...
#ifdef __APPLE__
  /* macOS: allocate one contiguous region for lowmem + HMA + extended memory.
     This ensures HOST_ADDR(dos_linear) works for all DOS addresses. */
  /* Add 16MB guard region so runaway CPU doesn't SIGBUS immediately.
     The real fix is preventing the CPU from executing in XMS space. */
  total_size = LOWMEM_SIZE + HMASIZE + EXTMEM_SIZE + (16 * 1024 * 1024);
  g_printf("macOS: allocating %zu bytes (lowmem+HMA+XMS) contiguously\n", total_size);
  result = alloc_mapping(MAPPING_INIT_LOWRAM, total_size, 0);
