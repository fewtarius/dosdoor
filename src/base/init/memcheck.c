...
void *dosaddr_to_unixaddr(void *addr)
{
  ...
  /* HMA range (0x100000-0x10FFFF) is mapped at lowmem_base + offset */
  if ((uintptr_t)addr >= MEM_SIZE && (uintptr_t)addr < (MEM_SIZE + HMASIZE))
    return LOWMEM(addr);
  if ((uintptr_t)addr >= (MEM_SIZE + HMASIZE))
    return addr;
