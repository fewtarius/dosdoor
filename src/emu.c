...
#endif
{
    ...
    /* Set CPU speed throttling - default 10 MHz for door game compatibility.
     * Set DOSDOOR_CPU_MHZ=0 to disable throttling (full speed). */
    if (config.cpu_mhz == 0) {
        const char *mhz_env = getenv("DOSDOOR_CPU_MHZ");
        if (mhz_env) {
            config.cpu_mhz = atoi(mhz_env);
            if (config.cpu_mhz == 0)
                config.cpu_mhz = -1;	/* -1 = explicitly disabled */
        }
        if (config.cpu_mhz == 0)
            config.cpu_mhz = 10;
    }
    if (config.cpu_mhz < 0)
        config.cpu_mhz = 0;		/* 0 = no throttle in interp.c */
    if (config.cpu_mhz)
        g_printf("CPU throttle: %d MHz\n", config.cpu_mhz);
    else
        g_printf("CPU throttle: disabled (full speed)\n");
#ifdef X86_EMULATOR
...
/* "graceful" shutdown */
static void leavedos_alarm_handler(int sig) {
    _exit(0);
}

static void *leavedos_watchdog(void *arg) {
    sleep(3);
    _exit(0);
    return NULL;
}
...
leavedos(int sig)
{
	...
	*/
    ...
    /* Hard 3-second deadline for remaining cleanup.
     * tcsetattr can block on macOS when not the foreground pgrp;
     * the watchdog thread catches any other hangs. */
    {
        ...
        sa.sa_handler = leavedos_alarm_handler;
        sigemptyset(&sa.sa_mask);
        sa.sa_flags = 0;
        sigaction(SIGALRM, &sa, NULL);

        deadline.it_interval.tv_sec = 0;
        deadline.it_interval.tv_usec = 0;
        deadline.it_value.tv_sec = 3;
        deadline.it_value.tv_usec = 0;
        setitimer(ITIMER_REAL, &deadline, NULL);

        pthread_create(&watchdog, NULL, leavedos_watchdog, NULL);
    ...
    if (config.cpuemu>1) {
    	g_printf("calling leave_cpu_emu\n");
    	leave_cpu_emu();
    }
#endif
    show_ints(0, 0x33);
