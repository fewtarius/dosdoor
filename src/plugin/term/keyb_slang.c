...
static void slang_keyb_close(void)  
{
	...
	/* Restore terminal settings. Skip if fd is not a terminal or
	 * if we're not the foreground process group (tcsetattr can
	 * block on macOS when another process controls the terminal). */
	if (keyb_state.kbd_fd >= 0 && isatty(keyb_state.kbd_fd)) {
		pid_t fg = tcgetpgrp(keyb_state.kbd_fd);
		if (fg == getpgrp()) {
			if (tcsetattr(keyb_state.kbd_fd, TCSANOW, &keyb_state.save_termios) < 0
			    && errno != EINVAL && errno != ENOTTY) {
